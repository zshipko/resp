...
module type WRITER = sig
  ...
  val encode: ?f:(oc -> 'a -> (int * (unit -> unit IO.t))) -> oc -> 'a t -> unit IO.t
end
...
module Reader (I: INPUT) = struct
  ...
  let rec decode ?f ic: lexeme -> 'a t IO.t =
    let f' = match f with
      | Some f -> (fun ic len -> f ic len >>= fun x -> IO.return @@ `Value x)
      | None -> (fun ic len -> read ic len >>= fun x -> IO.return @@ `String x)
    in
    function
    | `Nil -> IO.return `Nil
    | `Integer i -> IO.return @@ `Integer i
    | `Error e -> IO.return @@ `Error e
    | `String s -> IO.return @@ `String s
    | `Bs len -> f' ic len >>= fun b -> discard_sep ic >>= fun () -> IO.return @@ `Bulk b
    | `As len ->
...
module Writer (O: OUTPUT) = struct
  ...
  let rec encode ?f oc = function
    ...
    | `String s -> write_lexeme oc (`String s)
    | `Integer i -> write_lexeme oc (`Integer i)
    | `Bulk (`Value b) ->
      begin
        match f with
        | Some f ->
          let i, f = f oc b in write_lexeme oc (`Bs i) >>= fun () ->
          f () >>= fun () -> write_sep oc
        | None -> raise (Exc `Invalid_encoder)
      end
    | `Bulk (`String s) ->
...
module Bulk = struct
  module String (Reader: READER) (Writer: WRITER with module IO = Reader.IO) = struct
    module IO = Reader.IO
