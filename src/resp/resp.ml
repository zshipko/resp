...
module Reader (I : INPUT) = struct
  ...
  let rec read_lexeme ic : (lexeme, error) result Lwt.t =
    ...
    | c -> Lwt.return @@ Error (`Unexpected c)

  let rec decode ic : lexeme -> t Lwt.t = function
    | `Nil -> Lwt.return `Nil
    | `Integer i -> Lwt.return @@ `Integer i
    | `Error e -> Lwt.return @@ `Error e
    | `String s -> Lwt.return @@ `String s
    | `Bs len ->
        if len = 0 then Lwt.return (`Bulk "")
        else read ic len >>= fun b -> Lwt.return @@ `Bulk b
    | `As len ->
        if len = 0 then Lwt.return (`Array [||])
        else
  ...
  let rec encode oc = function
    ...
    | `Array a ->
        ...
        let rec write i =
          match i with
          | 0 -> Lwt.return ()
          | n -> encode oc a.(len - i) >>= fun () -> write (n - 1)
        in
