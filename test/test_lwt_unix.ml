...
let client, pid = Lwt_main.run begin
  match Unix.fork () with
  | -1 -> Printf.eprintf "Unable to fork process"; exit 1
  | 0 ->
      let server = `TCP (`Port 1234) in
      let data = Hashtbl.create 8 in
      let server = Server.create ~commands (Conduit_lwt_unix.default_ctx, server) data in
      Server.start server >|= fun () ->
      exit 0
  | pid ->
      Unix.sleep 1;
      let addr = Ipaddr.of_string_exn "127.0.0.1" in
      let params = (Conduit_lwt_unix.default_ctx, `TCP (`IP addr, `Port 1234)) in
...
let test_get _ () =
  Client.run_s client [| "get"; "abc" |] >>= Client.decode client >|= function
...
let () =
  Alcotest.run ~and_exit:false "Resp_lwt_unix" [
    "basic", basic
  ];
